...
``GET /__parsimony__/metrics`` when a stats source is wired.
"""
...
def render_prometheus(snapshot: dict[str, Any]) -> str:
    ...
    out: list[str] = []
    out += _family(
        "parsimony_requests_total",
        "counter",
        "Total proxied requests.",
        [({}, snapshot["requests"])],
    )
    out += _family(
        "parsimony_cache_hit_rate",
        "gauge",
        "Cache hit rate in [0,1].",
        [({}, snapshot["cache_hit_rate"])],
    )
    out += _family(
        "parsimony_tokens_saved_total",
        "counter",
        "Total input tokens saved.",
        [({}, snapshot["tokens_saved"])],
    )
    out += _family(
        "parsimony_overall_reduction_ratio",
        "gauge",
    ...
    if stages:
        ...
        if accuracy:
            out += _family(
                "parsimony_stage_accuracy_ratio",
                "gauge",
