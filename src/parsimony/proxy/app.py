...
Non-streaming requests go through :class:`~parsimony.proxy.engine.ProxyEngine` (compress +
...
def _meta_headers(meta: dict[str, Any]) -> dict[str, str]:
    """Render engine metadata as ``X-Parsimony-*`` response headers."""
    out: dict[str, str] = {}
    if "cache" in meta:
    ...
    if "tokens_before" in meta and "tokens_after" in meta:
        ...
        out["x-parsimony-tokens-after"] = str(after)
        out["x-parsimony-tokens-saved"] = str(max(0, before - after))
    return out
...
async def _stream_passthrough(
    ...
    if base is None:
        return JSONResponse(
            status_code=502,
            content={"error": "parsimony: unable to route request to a provider"},
        )
    ...
    response_headers["x-parsimony-cache"] = "stream-bypass"
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(title="parsimony", lifespan=lifespan)
