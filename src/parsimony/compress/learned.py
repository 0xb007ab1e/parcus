...
answer-preservation quality judge (``parsimony.eval.quality.LLMJudge``) against the eval corpus
    ...
    def reduce(self, text: str, *, keep_ratio: float) -> str:  # pragma: no cover - needs model
        ...
        if self._compressor is None:
            try:
                ...
                raise ImportError(
                    "the learned tier requires the 'learned' extra: "
                    "pip install 'parsimony[learned]'"
                ) from exc
