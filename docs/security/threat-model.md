...
| I2 | PII/secret in persisted cache or graph (I) | TB3 | **Redact-before-persist** (default on); TTL expiry; no-cache regex patterns; kill switch; in-memory-only mode option. Hosted: **per-tenant DEKs** (HKDF per tenant) bound the blast radius of a key compromise, and **crypto-shredding** (withhold a tenant's key) gives right-to-erasure without row scans (ADR 0007). |
